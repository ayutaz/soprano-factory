"""
End-to-end decoder training script with GAN.

This script trains a decoder that directly accepts 512-dim hidden states
(matching Soprano's LLM output) instead of mel spectrograms.

Architecture:
    LLM hidden states (512-dim) → VocosDecoder → Audio

This enables direct integration with Soprano TTS without post-processing.
"""
...
class HiddenStateSimulator(nn.Module):
    """Simulate LLM hidden states from mel spectrogram.

    In real Soprano inference, the LLM outputs 512-dim hidden states.
    For training, we simulate these by projecting mel spectrograms to 512-dim.
    This allows the decoder to learn the 512-dim → audio mapping.
    """
    def __init__(self, n_mels: int = 100, hidden_dim: int = 512):
        super().__init__()
        # Project mel to hidden state dimension
        self.projection = nn.Conv1d(n_mels, hidden_dim, kernel_size=1)
    ...
    def forward(self, mel: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            mel: (B, n_mels, T)
        Returns:
            hidden_states: (B, hidden_dim, T)
        """
        return self.projection(mel)


class VocosDecoderE2E(nn.Module):
    """Vocos decoder that accepts 512-dim hidden states (matching Soprano).

    Architecture matches Soprano's SopranoDecoder but with GAN training.
    """
    def __init__(
        self,
        input_dim: int = 512,  # Match LLM hidden dim
        dim: int = 512,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            audio: (B, T_audio)
        """
        # Linear interpolation (same as SopranoDecoder)
        x = torch.nn.functional.interpolate(
            x, scale_factor=self.upscale, mode='linear', align_corners=True
        )
        x = self.backbone(x)
        audio = self.head(x)
        return audio
...
class TrainConfig:
    ...
    use_wandb: bool = True
    wandb_project: str = "soprano-decoder-e2e"
...
def train(config: TrainConfig):
    ...
    hidden_simulator = HiddenStateSimulator().to(device)
    generator = VocosDecoderE2E().to(device)
    ...
    # Optimizers (train both hidden_simulator and generator together)
    ...
    while step < config.max_steps:
        ...
        # Extract mel spectrogram and simulate hidden states
        with torch.no_grad():
            mel = mel_extractor(audio)

        # Simulate hidden states (this is trained)
        hidden_states = hidden_simulator(mel)

        # Generate audio from hidden states
        audio_hat = generator(hidden_states)
        ...
        torch.nn.utils.clip_grad_norm_(hidden_simulator.parameters(), 1.0)
        opt_gen.step()
        ...
        if step % config.save_freq == 0:
            ...
            torch.save({
                'hidden_simulator': hidden_simulator.state_dict(),
    ...
    # Save final model
    final_path = os.path.join(config.save_dir, "decoder_e2e.pth")
    torch.save({
        'hidden_simulator': hidden_simulator.state_dict(),
        'generator': generator.state_dict(),
    }, final_path)
...
def main():
    parser = argparse.ArgumentParser(description="Train end-to-end Vocos decoder with GAN")
    ...
    parser.add_argument("--wandb-project", type=str, default="soprano-decoder-e2e")
