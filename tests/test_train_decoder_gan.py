"""Tests for train_decoder_gan.py"""
...
class TestImports:
    ...
    def test_import_mel_extractor(self):
        from train_decoder_gan import MelSpecExtractor
        extractor = MelSpecExtractor()
        assert extractor is not None

    def test_import_vocos_decoder(self):
        from train_decoder_gan import VocosDecoder
        decoder = VocosDecoder()
        assert decoder is not None

    def test_import_discriminators(self):
        from train_decoder_gan import MultiPeriodDiscriminator, MultiResolutionDiscriminator
...
class TestMelSpecExtractor:
    ...
    def test_forward_shape(self):
        from train_decoder_gan import MelSpecExtractor
        extractor = MelSpecExtractor()
        audio = torch.randn(2, 32000)  # 1 second at 32kHz
        mel = extractor(audio)
        assert mel.dim() == 3
        assert mel.size(0) == 2
...
class TestVocosDecoder:
    """Tests for VocosDecoder."""
...
class TestDiscriminators:
    """Tests for discriminators."""

    def test_mpd_forward(self):
        from train_decoder_gan import MultiPeriodDiscriminator
        mpd = MultiPeriodDiscriminator()
    ...
    def test_mrd_forward(self):
        from train_decoder_gan import MultiResolutionDiscriminator
        mrd = MultiResolutionDiscriminator()
        y = torch.randn(2, 16000)
        y_hat = torch.randn(2, 16000)
        y_d_rs, y_d_gs, fmap_rs, fmap_gs = mrd(y, y_hat)
        assert len(y_d_rs) == 3  # 3 resolutions
        assert len(y_d_gs) == 3
...
class TestLosses:
    ...
    def test_multi_scale_mel_loss(self):
        from train_decoder_gan import MultiScaleMelLoss
    ...
    def test_generator_loss(self):
        from train_decoder_gan import GeneratorLoss
    ...
    def test_discriminator_loss(self):
        from train_decoder_gan import DiscriminatorLoss
    ...
    def test_feature_matching_loss(self):
        from train_decoder_gan import FeatureMatchingLoss
...
class TestDecoderDataset:
    ...
    def test_dataset_creation(self, tmp_path):
        from train_decoder_gan import DecoderDataset
...
class TestTrainConfig:
    ...
    def test_default_values(self):
        from train_decoder_gan import TrainConfig
        config = TrainConfig(
...
class TestEndToEnd:
    ...
    def test_training_step(self):
        ...
        from train_decoder_gan import (
            MelSpecExtractor, VocosDecoder,
        ...
        # Create optimizers
        opt_gen = torch.optim.AdamW(generator.parameters(), lr=1e-4)
